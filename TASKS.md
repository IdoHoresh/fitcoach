...
- [ ] Remove dev-only buttons from Profile tab — **defer until v1.0 launch gate** (pre-release cleanup only; useful for in-dev testing until then)
...
- [x] **Recent foods as default (2026-04-23)** — 15 most recently logged foods shown with a "לאחרונה" section header before the user types. Zero-history users silently fall through to `search('', 15)` (no label) — the v16 source-tier ranker surfaces `raw_` items first, so brand-new users see Israeli staples (חזה עוף / ביצה / אורז / שמן זית / תפוח) instead of an arbitrary catalog slice. Discovery: `foodRepository.getRecent(limit = 15)` was already wired to `FoodSearchSheet` via an empty-query branch — the feature was invisible because there was no section label. Net change: `resultSource: 'recent' \| 'search' \| null` state + conditional `<Text testID="…-recent-label">` + dropped explicit `getRecent(30)` arg + fallback `search('', 50)` → `search('', 15)` + flipped Hebrew i18n value `'נפוצים לאחרונה'` → `'לאחרונה'` (reused dead key). 4 files, 75 insertions / 9 deletions, no schema change. +3 new tests (label visible on recent, hidden on fallback, hidden on typing) + 1 updated `getRecent`-arg assertion. 2,332 tests.
- [x] **One-tap re-log previous meal (2026-04-23)** — Empty `MealSection` renders an inline pill "אותה ארוחת בוקר כמו אתמול →" (or day-of-week if not literally yesterday); one tap clones every prior-meal entry to today at identical grams. 7-day walkback per mealType, strict `date < selectedDate` exclusion. Transactional clone via new `FoodLogRepository.cloneEntriesToDate`. Toast "הוספו N פריטים · בטל" with 5s dwell; tap "בטל" calls new transactional `deleteManyByIds`. Reuses the existing `redistributionToast` animation pattern (duplicate block, different dwell + undo action). Per-meal lookup moved into store as `previousMealSourceDates` / `loadPreviousMealLookup` to keep DB access centralized. UX: undo-last semantics (consecutive re-logs replace, don't merge — matches iOS/Gmail). 9 files, +619 LoC including tests. +26 new tests (repo +10, store +9, pill +3, MealSection +4). 2,358 tests. Spec at `docs/specs/2026-04-23-one-tap-relog-previous-meal.md`.
...
- [x] A1 Settings toggle — meal-logging mode flip post-onboarding (2026-04-27) — `src/components/settings/{SettingsRow,ModeToggleSheet}.{tsx,test.tsx}` (new dir). `SettingsRow` is a generic primitive (label + optional subtitle + onPress + testID, RTL-aware via framework `I18nManager.isRTL`, `useAnimatedPress` + light haptic). `ModeToggleSheet` wraps two `ModeCard`s in a slide-up `Modal`, reusing `ModeCard` + `ModeInfoSheet` + previews from PR #85 — info sheet nested INSIDE the toggle sheet's Modal per sibling-Modal lesson (2026-04-18), single `infoMode: MealLoggingMode | null` state swaps content. Tap card → `onSelect(mode); onClose()` in that order; tap backdrop → `onClose()`; tap ⓘ → info sheet without commit. `app/(tabs)/profile.tsx` adds a `Settings section` above dev tools, subscribes to `useUserStore((s) => s.profile?.mealLoggingMode)`, fires `void updateProfile({ mealLoggingMode })` on switch (skipped when `next === currentMode` to avoid no-op DB writes), no confirm dialog (non-destructive — logs persist; only future entry flow changes). i18n keys `settings.mealLoggingMode.{title,subtitleStructured,subtitleFree,sheetTitle}` in he + en, reuses existing `onboarding.modeChoice.*` for cards + info sheets. 11 new tests (4 SettingsRow + 7 ModeToggleSheet — backdrop test uses `screen.UNSAFE_root.findAll` because `accessibilityViewIsModal` siblings hide from `screen.getByTestId`; lessons updated). Manual sim verify deferred: `expo start --ios` deeplink times out on this worktree's sim (dev client app not installed), and Task 4 is pure UI wiring with no algorithmic risk surface — covered by 11 tests + typecheck. Analytics deferred to A1-analytics (no infra yet). Spec: `docs/specs/2026-04-27-settings-mode-toggle.md`. 2,368 tests total. Closes Track A1 of two-mode meal logging.
...
- [x] **A1 — Mode preference + onboarding mode-choice screen** (M, 2026-04-27) — Add `mealLoggingMode: 'structured' \| 'free'` to `UserProfile`, SQLite v20 migration, `app/(onboarding)/mode-choice.tsx` (stacked cards, intent framing, Structured default-selected), Settings toggle + i18n.
  - 2026-04-24 partial ship: mode-choice screen + `UserProfile.mealLoggingMode` type + SQLite v20 migration + i18n (he/en) + `ModeCard` / `ModeInfoSheet` / `StructuredModePreview` / `FreeModePreview` components landed (PR #85).
  - 2026-04-27 final ship: Settings toggle landed — `SettingsRow` primitive + `ModeToggleSheet` (nests `ModeInfoSheet` per sibling-Modal lesson) + Profile-tab wiring + i18n keys (`settings.mealLoggingMode.*`). 11 new tests, 2,368 total. Spec: `docs/specs/2026-04-27-settings-mode-toggle.md`. **practice-meal screen moved to Track F (F5)** — circular dep: practice-meal demonstrates the Structured wizard (Track B) OR Free flow (Track C), so it can't build until B/C exist.
- [x] **A1-analytics — Default-select safety-net analytics** (S, 2026-04-27) — Vendor-agnostic stub `src/analytics/track.{ts,test.ts}` (`__DEV__` console.log, prod no-op, swap impl when vendor picked post-P0.4) with discriminated-union `AnalyticsEvent` for compile-time prop safety. Two events wired: (1) `mode_choice_picked` (mode + `time_to_pick_ms` from `useRef(Date.now())` mount clock + `changed_from_default` final-vs-default semantics) fires in `app/(onboarding)/mode-choice.tsx` `handleContinue`; (2) `mode_switched_in_settings` (from/to + `days_since_onboarding` derived from existing `profile.createdAt` — no schema bump) fires in `app/(tabs)/profile.tsx` `handleModeSelect` after `updateProfile`, skipped on no-op writes. `jest.setup.ts` gains `expo-linear-gradient` mock (other screen tests will reuse). 14 new tests (5 helper DEV/prod/shape + 5 mode-choice including `useFakeTimers` time-elapsed assertion + 4 profile including same-day=0 day boundary). Spec: `docs/specs/2026-04-27-a1-analytics.md`. 2,382 tests total. Alert threshold from parent spec: Structured >90% of onboarding completions → rubber-stamp signal.
- [x] **A2 — Shared slider primitive** (L, 2026-04-30) — `src/components/nutrition/shared/Slider.tsx` with dual labels (`125 גר׳ · ½ חזה` on tick / `≈ 0.7 חזה · 145 גר׳` between), tick marks at natural units, haptic snaps via reanimated worklet + RNGH Pan gesture, cooked/raw toggle (parent owns sibling lookup; grams preserved on swap), 3 quick-tap portion pills above track, hand-portion icon. Slug-keyed `SERVING_TICKS` const map at `src/data/serving-ticks.ts` with 20 starter entries (chicken raw+cooked pair, egg, greek yogurt, cottage, tuna, rice, bread, pita, oats, pasta, olive oil, avocado, tahini, milk, white cheese, banana, apple, tomato, almonds). Pure helpers `detectTickCrossings` / `getServingTicks` / `getCookedVariant` / `getHandPortion` with DI-friendly tickMap param + 17 unit tests + 6 data-integrity tests. Slider has 14 render tests covering happy path, fallback, clamping, empty-ticks, and a11y. Adds `react-native-gesture-handler ~2.28.0` + `GestureHandlerRootView` at root layout. Spec: `docs/specs/2026-04-30-shared-slider-primitive.md`. PR-A (data, #89): types + slug + helpers + data + tests, CI green. PR-B (component): Slider skeleton + gesture + haptic + a11y polish, opening next. Tests: 2,382 → 2,419.
...
- [x] **Tiv Taam Phase 1 — transparency-feed catalog gap analysis (2026-04-21)** — `scripts/{transparency-feed-types,parse-transparency-feed,filter-food-items,download-tivtaam-feed,build-tivtaam-catalog}.ts` + `scripts/certs/sectigo-public-server-auth-ca-dv-r36.pem` + npm scripts. Pipeline: auth to `url.publishedprices.co.il` as `TivTaam`/empty pw → fetch newest `PriceFull*.gz` → gunzip + fast-xml-parser → drop non-food (19 Hebrew keyword blacklist) → classify each item as `net-new` / `in-shufersal` / `in-rami-levy` / `no-ean` by barcode dedup → write `tmp/tivtaam-catalog.json` + print summary. TDD on the two pure functions (22 tests: 10 parser incl. lowercase/alias regression, 12 filter). Manual-verify on download (auth flow + cache + --force + --dry-run + staleness warning). Discovered + fixed two non-obvious infra issues: (1) publishedprices server ships only leaf cert, no intermediate — bundled Sectigo DV R36 PEM via `NODE_EXTRA_CA_CERTS` since Node's fetch doesn't AIA-chase; (2) native `fetch` drops cookies across redirects and the login 302 rotates the session cookie, so redirects are handled manually. **Net-new: 10,024 items (5× over the 2,000 threshold), 2,797 imported (moat confirmed — Beyond Meat, Milano cookies, Joseph Drouhin wines, Kraft mac & cheese).** Phase 2 is a clear go. Spec: `docs/specs/2026-04-21-tiv-taam-phase1-catalog-gap.md`. Detailed lessons in `lessons.md`. 2,306 tests (+37 over 2,269 baseline from the new scraper suite plus unrelated gains since baseline measurement).

- [x] **Tiv Taam Phase 2 — OFF nutrition enrichment + v19 seed migration (2026-04-22)** — shipped `src/assets/tivtaam-seed.json` with **1,920 tt\_ rows (687 imported-exclusive)** after a 6-pass OFF fetch against the 9,548 net-new EANs from Phase 1. Pipeline: `scripts/fetch-tivtaam-off.ts` resumable cache with 1500ms gap + 2500ms retry-errors pass → `scripts/build-tivtaam-seed.ts` reads cache + Phase 1 catalog, normalizes hits via `normalizeOffProduct({ idPrefix: 'tt' })`, drops rows where `protein+fat+carbs === 0` (877 OFF-indexed-but-no-macros items routed to review queue instead of shipping misleading 0g macros). `src/services/open-food-facts.ts` refactor exposes pure `fetchRawOffProduct` so the seed builder caches raw JSON and re-normalizes with the tt\_ prefix. v19 migration follows the v14/v15/v16 DELETE-then-INSERT-OR-IGNORE pattern: ALTER TABLE foods ADD `origin_country TEXT` (nullable — populated only for tt\_ rows, NULL for sh\_/rl\_/raw\_/manual\_) → DELETE tt\_% → batch INSERT OR IGNORE. tt\_ rows slot into tier 4 (lowest) of v17 name_norm cross-source cleanup via existing CASE else branch. Spec: `docs/specs/2026-04-21-tiv-taam-phase2-off-enrichment.md`. +18 new tests (15 seed-builder + 2 normalizer idPrefix + 1 v19 schema column). 2,328 tests total.
  - [ ] **Follow-up: re-fetch persistent errors (scheduled 2026-04-23 morning)** — 1,618 EANs from the Phase 2 main fetch are still uncached (OFF CDN rate-limit errors that even the 2500ms retry pass couldn't resolve). Running `npm run fetch-tivtaam-off` fresh will re-attempt only those 1,618 (cache entries skip the rest). Expected recovery ~60-70% = ~1,000 new cache entries → after zero-macros filter, ~480-700 additional seed rows (total ~2,400-2,600 tt\_ rows). Pipeline: `npm run fetch-tivtaam-off` → `npm run build-tivtaam-seed` → commit refreshed `src/assets/tivtaam-seed.json`. ~3 hours background. No schema change, no UI change, no PR review needed beyond the seed diff.
  - [x] **Task 0 — OFF hit-rate probe (2026-04-21)** — `scripts/fetch-tivtaam-off.ts` (new, `--probe` stub) + `scripts/transparency-feed-types.ts` (25 new `NON_FOOD_KEYWORDS` — cosmetics, cleaning brands, pet food, kitchenware, tobacco, charcoal). Stratified 100-item sample (50 domestic + 50 imported) × 3-pass error retry (500ms → 1500ms → 2500ms gap) drove errors to zero. **Overall 31% hit rate (17 full + 14 partial), imported slice 40% (11 full + 9 partial).** Catalog shrank 10,024 → 9,548 after filter tightening. Projected Phase 2 ship: ~2,960 items, of which ~1,080 imported-exclusive (moat ~75% larger than initial estimate). Deep-dive ruled out EAN-variant and OFF-search-by-name as lift paths (OFF normalizes barcodes internally; search API returns 0 results for niche imports). Task 3 full fetcher will use **1500ms floor + automatic retry-errors pass at 2500ms**, not the originally-spec'd 500ms. Full fetch duration revised from ~90 min → ~4 hours. Decision gate met in the "confirm with user" band (20–34%); user approved proceed. Lessons recorded in `lessons.md` (Tiv Taam Phase 2 Task 0 section).

- [ ] **Add more Israeli supermarkets beyond Tiv Taam** — Victory, Yohananof, Am-Pm, Osher Ad. All use the same Price Transparency Law feed format as Tiv Taam, so the parser + filter written in Tiv Taam Phase 1 are directly reusable (promote to `scripts/transparency/` subdirectory when the 2nd chain is added). Deferred until Tiv Taam Phase 1 + 2 ship and usage data confirms the pattern is worth repeating.
