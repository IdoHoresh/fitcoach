...
- [ ] **Tiv Taam Phase 2 — OFF nutrition enrichment + v19 seed migration** — unlocked by Phase 1 result. For each `net-new` item in `tmp/tivtaam-catalog.json` with a valid EAN, call `fetchOffProduct(ean)` (exists from PR #76), normalize OFF hits into `FoodSeed` with `id: tt_<barcode>`, extend `foods` schema with `origin_country TEXT` column (v19 migration, follows v14/v15 pattern), write `tt_` rows to `src/assets/tivtaam-seed.json`, drop no-hit items. Spec: `docs/specs/2026-04-21-tiv-taam-phase2-off-enrichment.md`.
