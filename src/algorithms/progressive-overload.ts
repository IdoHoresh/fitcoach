...
const SMALL_UPPER_MUSCLES: ReadonlySet<MuscleGroup> = new Set(['biceps', 'triceps', 'shoulders'])

const SMALL_LOWER_MUSCLES: ReadonlySet<MuscleGroup> = new Set(['calves'])
 ...
 */
export function allSetsAtTopOfRange(sets: readonly LoggedSet[], maxReps: number): boolean {
 ...
 */
export function hasPerfDeclined(sets: readonly LoggedSet[], minReps: number): boolean {
  const workingSets = sets.filter((s) => !s.isWarmup)
...
export function getProgressionAdvice(
  ...
  if (consecutiveDeclines >= MESOCYCLE.DECLINE_THRESHOLD) {
    const deloadWeight =
      Math.round(currentWeightKg * (1 - MESOCYCLE.DELOAD_WEIGHT_REDUCTION) * 2) / 2 // Round to nearest 0.5kg
