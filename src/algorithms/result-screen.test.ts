...
describe('Result screen computations', () => {
  ...
  describe('displayed calories must be goal-adjusted (not maintenance TDEE)', () => {
    it('fat_loss: targetCalories is less than TDEE total', () => {
      const { tdeeBreakdown, nutrition } = computeResults(buildCompleteDraft({ goal: 'fat_loss' }))

      // The UI must display nutrition.targetCalories, NOT tdeeBreakdown.total
      expect(nutrition.targetCalories).toBeLessThan(tdeeBreakdown.total)
      expect(tdeeBreakdown.total - nutrition.targetCalories).toBeGreaterThanOrEqual(300)
      expect(tdeeBreakdown.total - nutrition.targetCalories).toBeLessThanOrEqual(500)
    })

    it('muscle_gain: targetCalories is more than TDEE total', () => {
      const { tdeeBreakdown, nutrition } = computeResults(
        buildCompleteDraft({ goal: 'muscle_gain' }),
      )

      expect(nutrition.targetCalories).toBeGreaterThan(tdeeBreakdown.total)
      expect(nutrition.targetCalories - tdeeBreakdown.total).toBeGreaterThanOrEqual(200)
      expect(nutrition.targetCalories - tdeeBreakdown.total).toBeLessThanOrEqual(300)
    })

    it('maintenance: targetCalories equals TDEE total', () => {
      const { tdeeBreakdown, nutrition } = computeResults(
        buildCompleteDraft({ goal: 'maintenance' }),
      )

      expect(nutrition.targetCalories).toBe(tdeeBreakdown.total)
    })

    it('female fat_loss scenario matches expected range', () => {
      // The exact scenario that caught the bug:
      // Female, 30yo, 59.5kg, 160cm, desk job, 2x/week aerobic+weights
      const draft = buildCompleteDraft({
        sex: 'female',
        age: 30,
        weightKg: 59.5,
        heightCm: 160,
        goal: 'fat_loss',
        trainingDays: [0, 3] as readonly DayOfWeek[],
        experience: 'beginner',
        lifestyle: {
          occupation: 'desk',
      ...
      // TDEE should be ~1,700-1,800 (maintenance)
      expect(tdeeBreakdown.total).toBeGreaterThan(1600)
      expect(tdeeBreakdown.total).toBeLessThan(1900)

      // Target calories should be TDEE - 400 (deficit), NOT the TDEE itself
      expect(nutrition.targetCalories).toBeLessThan(tdeeBreakdown.total)
      expect(nutrition.targetCalories).toBeGreaterThanOrEqual(1200) // female floor
      expect(nutrition.targetCalories).toBeLessThan(1600)
    })
  })
