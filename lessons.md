...
- **Algorithm tests must pin at least one input to a published reference value; self-asserting tests miss sign errors and coefficient typos.** The Mifflin-St Jeor male offset was `-5` instead of `+5` for ~6 weeks. The unit test (`calculates_male_bmr_correctly`) asserted `1770` — the value the broken code returned — not `1780`, the canonical literature value (Mifflin 1990, AJCN). Tests written after the implementation pin to the implementation, not the spec. Rule: for any function reproducing a published formula (BMR, TDEE, EWMA, BMI, FFMI, body-fat math), include at least one test named `matches_published_<source>_canonical_<case>` with a comment showing the formula evaluation and the journal citation. **Sweep instruction:** any test of the form `expect(fn(...)).toBe(<value>)` where `<value>` was computed by running the function once should be rewritten to pin to a published reference. If a code change breaks the canonical test, the code is wrong — not the test.

- **Specs and code drift in both directions; reviews touching algorithmic correctness must cite line numbers, not spec sections.** Three findings in one review thread were wrong because the reviewer cited the spec instead of the code: the protein/LBM critique missed that the code already implements Helms adjusted-weight; the macro-distributor citation critique missed that the code already cites Schoenfeld 2018 + Mamerow 2014; the "two-step cadence" finding was right because the spec described EWMA-style smoothing while the code did a 2-point delta. The fix is asymmetric — when the code looks better than the spec describes, fix the spec; when the code looks worse, fix the code. Either way, code citations with line numbers, not spec sections.

- **Single-line fixes to math primitives have non-local effects; verification scope is the algorithm graph, not the changed line.** The BMR sign fix was one character; the verification scope was every downstream consumer (TDEE → target calories → macro splits → recalibration delta). Before claiming "trivial" for a change to a primitive: (1) `grep` for callers and enumerate consumers, (2) check whether the change crosses any threshold or tipping point in those consumers (recalibration trigger, calorie floor, macro-floor protection, severity ladder bins), (3) confirm at least one canonical-value test pins the function to a published reference. "10 kcal is below the recalibration threshold" is the convenience-biased verdict, not the verified one.

- **When a spec describes a differentiator and the code doesn't implement it, that's a product problem, not a documentation problem.** The recalibration code's "20-day EWMA on weight trend" promised by the spec was a 2-point weekly delta. Framing this as "spec divergence" implies symmetric options (rip code OR amend spec). For a marketed differentiator (adaptive-TDEE EWMA vs. MyFitnessPal), the options aren't symmetric — downgrading the spec means publicly admitting a worse algorithm than every competitor. Frame these findings as "the moat doesn't exist yet," not "spec needs an update." Forces a real product decision instead of a doc-update task.

- **Review severity downgrades that come from a simpler frame should re-derive the finding from first principles before lowering priority.** Convenience bias (§4.5) fires not only on test verdicts but on severity ratings, scope decisions, rip-vs-amend choices. The "% of TDEE for goal deltas" finding was downgraded after the audit noticed code uses −300 to −500 instead of the reviewer's assumed −750 — but the original argument was about shape (a 500-kcal deficit is 28% for a 60kg woman, 17% for a 110kg man), not magnitude. The shape problem doesn't go away when the upper bound shrinks. Always re-derive the argument structure before downgrading; if the simpler frame loses load-bearing detail, the simpler frame is wrong.

- **Audits accept the easy framing of the previous layer's findings unless they re-derive.** The audit took "% of TDEE" magnitude difference as the finding's substance and concluded "less aggressive than reviewer assumed → lower priority." It didn't re-read the original argument. The fix is to read the actual argument structure of each upstream finding before adjusting its priority. Path-of-least-cognitive-load is the pattern; explicit re-derivation is the antidote.

- **Each layer of a multi-layer review must read the actual artifact, not just the previous layer's summary.** Multi-layer reviews compound bias proportional to how much each layer relies on the previous layer's framing. The audit caught two errors in the previous review because it read the code; the red-team caught two underweightings in the audit because it re-read the same code. Short-circuiting ("previous reviewer already read it") is the moment bias compounds. Rule: at every review layer — even the third or fourth — open the file, read the lines, then write the finding.
