...
- **`getByBarcode` is the ONLY guard against `manual_<ean>` collision.** `insertFood` uses `INSERT OR REPLACE` at [food-repository.ts:166](src/db/food-repository.ts:166), so a second call for the same EAN silently overwrites. In the scanner flow this is fine — `getByBarcode` in the sheet's scan handler runs before the manual form can open, and it checks all four tiers (`raw_`, `manual_`, `sh_`, `rl_`). Any future entry point that bypasses that pre-check (text-search create, edit-food flow, deep-link) MUST add its own `foodRepository.getById("manual_<ean>")` collision check before calling `insertFood`, with explicit overwrite confirmation. `INSERT OR REPLACE` is a footgun for generic manual writes; the OFF partial-data refresh path is the only legitimate consumer of the overwrite semantics.
...
- **Zod error tokens let the schema stay locale-agnostic.** `z.string().min(1, 'nameRequired')` + `refine((d) => ..., { message: 'MACRO_SUM_TOO_HIGH' })` emits stable i18n-key tokens. The form maps `errors.<field>` → `strings.errors[token]` at render time, so the schema never imports `@/i18n` and can be reused from non-UI callers (scripts, tests) without locale setup.
