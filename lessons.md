...
- **Algorithm functions implementing published formulas need at least one test that pins to the literature value, not whatever the code happens to compute.** The Mifflin-St Jeor male offset was `-5` instead of `+5` for ~6 weeks. The unit test (`calculates_male_bmr_correctly`) asserted `1770` — the value the broken code returned — not `1780`, the canonical literature value (Mifflin 1990, AJCN). Tests-after-implementation pin to the implementation, not the spec. Pattern: for any function reproducing a published formula (BMR, TDEE, EWMA, BMI, FFMI), include at least one test named `matches_published_<source>_canonical_<case>` with a comment showing the formula evaluation and the journal/source citation. If a code change breaks that test, the code is wrong — not the test. (2026-05-10)
