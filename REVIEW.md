...
- **Repository invariants belong in API verbs, not in lessons.** When two methods have meaningfully different semantics (upsert vs strict insert, overwrite-ok vs overwrite-forbidden), split them into two exports with different names. The verb at the call site IS the invariant — a future contributor can't pick the wrong one without seeing the mismatch. Tagged-error classes can carry a payload (`FoodCollisionError.existing: FoodItem`) so callers skip re-querying. (2026-04-18)
- **Sibling Modals z-fight on iOS — nest them.** Two `{cond && <Modal/>}` blocks as siblings under the same parent produce unreliable z-ordering on iOS. Render the secondary Modal INSIDE the primary Modal's content tree (nested) rather than alongside it. State machine still drives both; only the JSX location changes. (2026-04-18)
